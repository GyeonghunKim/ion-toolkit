...
class EnergyLevel:
    def __init__(
        self, energy: float, n: int, I: Rational, L: int, J: Rational, line_width: float
    ):
        self.energy = energy
        self.n = n
        self.I = I
...
class FineStructureZeemanLevel(EnergyLevel):
    def __init__(
        ...
        self.zeeman_splitting_func: Callable[[float], float] = (
            lambda B_field: self.lande_g_factor * self.m_J * Constants.mu_B * B_field
        )
...
class HyperfineStructureZeemanLevel(EnergyLevel):
    def __init__(
        ...
        self.zeeman_splitting_func: Callable[[float], float] = (
            lambda B_field: self.lande_g_factor * self.m_F * Constants.mu_B * B_field
        )
    ...
    def __init__(
        ...
        self.zeeman_levels = [
            FineStructureZeemanLevel(
                self.energy, self.n, self.I, self.L, self.J, m_J, self.line_width
            )
